...
install_naive() {
    log_step "安装 NaiveProxy（klzgrad/naiveproxy）..."
    ...
    if ! command -v caddy-naive &>/dev/null; then
        log_error "NaiveProxy 安装失败"
        exit 1
    fi

    log_info "NaiveProxy 安装成功: ${latest_release}"
...
[Unit]
Description=NaiveProxy (Caddy with forwardproxy)
